...
public abstract class ImageGL implements Image {
  ...
  void draw(GLShader shader, InternalTransform xform, float dx, float dy, float dw, float dh,
            boolean repeatX, boolean repeatY, float red, float green, float blue, float alpha) {
    ...
    if (tex > 0) {
      ...
      ctx.quadShader(shader).prepareTexture(tex, red, green, blue, alpha).addQuad(
  ...
  private void scaleTexture(boolean repeatX, boolean repeatY) {
    ...
    try {
      ...
      ctx.clear(0, 0, 0, 0);
      ctx.quadShader(null).prepareTexture(tex, 1, 1, 1, 1).addQuad(
