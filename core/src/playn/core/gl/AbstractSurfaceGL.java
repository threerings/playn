...
abstract class AbstractSurfaceGL implements Surface {
  ...
  public Surface fillTriangles(float[] xys, int[] indices) {
    return fillTriangles(xys, 0, xys.length, indices, 0, indices.length);
  }
    ...
    if (fillPattern != null) {
      ...
      if (tex > 0) {
        ...
        shader.addTriangles(topTransform(), xys, xysOffset, xysLen,
                fillPattern.width(), fillPattern.height(), indices, indicesOffset, indicesLen);
      }
    } else {
      int tex = ctx.fillImage().ensureTexture();
      shader.prepareTexture(tex, Tint.combine(fillColor, tint));
      shader.addTriangles(topTransform(), xys, xysOffset, xysLen,
              1, 1, indices, indicesOffset, indicesLen);
    }
    return this;
  }
  ...
  public Surface fillTriangles(float[] xys, float[] sxys, int[] indices) {
    return fillTriangles(xys, sxys, 0, xys.length, indices, 0, indices.length);
  }
  ...
  public Surface fillTriangles(float[] xys, float[] sxys, int xysOffset, int xysLen,
    ...
    if (tex > 0) {
      ...
      shader.addTriangles(topTransform(), xys, sxys, xysOffset, xysLen, indices, indicesOffset, indicesLen);
    }
    return this;
  }
