...
class JavaGLESContext extends GL20Context implements JavaGLContextInterface {

  public static final boolean CHECK_ERRORS = Boolean.getBoolean("playn.glerrors");

  JavaGLESContext(JavaPlatform platform, float scaleFactor, int screenWidth, int screenHeight) {
    super(platform, new JavaGLES20(), scaleFactor, CHECK_ERRORS);
  ...
  public void updateTexture(int tex, BufferedImage image) {
    ...
    bindTexture(tex);
    GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, image.getWidth(),
        image.getHeight(), 0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, buf);
