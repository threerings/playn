...
public class JavaLWJGLEmulatedTouch extends JavaEmulatedTouch {
    ...
    JavaMouse createMouse (JavaPlatform platform) {
        return new JavaLWJGLMouse(platform) {
            ...
            @Override protected boolean onMouseDown(Mouse.ButtonEvent.Impl event) {
                JavaLWJGLEmulatedTouch.this.onMouseDown(event.time(), event.x(), event.y());
                return false;
            }

            @Override protected boolean onMouseMove(Mouse.MotionEvent.Impl event) {
                JavaLWJGLEmulatedTouch.this.onMouseMove(event.time(), event.x(), event.y());
                return false;
            }

            @Override protected boolean onMouseUp(ButtonEvent.Impl event) {
                JavaLWJGLEmulatedTouch.this.onMouseUp(event.time(), event.x(), event.y());
                return false;
            }
